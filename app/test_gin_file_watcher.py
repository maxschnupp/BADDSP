...
class TestGinFileWatcher(unittest.TestCase):
    ...
    def test_gin_file_watcher(self):
        ##  should match and trigger for files ending in .gin in the folder
        ...
        ##  should match and trigger for files ending in checkpoint in the folder
        with mock.patch('gin_file_watcher.on_created') as on_created:
            file_thread = gin_file_watcher.get_watchdog_thread()
            file_thread.start()
            os.system('> {}/someInstrument-checkpoint'.format(constants.SAVE_DIR))
            time.sleep(4)
            self.assertTrue(on_created.called)
        ##  should match and trigger for files containing ckpt in the name in the folder
        with mock.patch('gin_file_watcher.on_created') as on_created:
            file_thread = gin_file_watcher.get_watchdog_thread()
            file_thread.start()
            os.system('> {}/blahlahhhckptblahblahblah'.format(constants.SAVE_DIR))
            time.sleep(4)
            self.assertTrue(on_created.called)
